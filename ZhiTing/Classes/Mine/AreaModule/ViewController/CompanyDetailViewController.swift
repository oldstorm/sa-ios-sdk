...
class CompanyDetailViewController: BaseViewController {
    lazy var requestQueue = DispatchQueue(label: "ZhiTing.CompanyDetailViewController.requestQueue")
    ...
    private lazy var departmentsNumCell = ValueDetailCell().then {
    ...
    private lazy var deleteButton = ImageTitleButton(frame: .zero, icon: nil, title: "解散公司".localizedString, titleColor: UIColor.custom(.black_333333), backgroundColor: UIColor.custom(.white_ffffff)).then {
        $0.isHidden = true
        $0.clickCallBack = { [weak self] in
            guard let self = self else { return }
            
            let str0 = getCurrentLanguage() == .chinese ? "确定解散吗?\n\n" : "Are you sure to dismiss it?\n\n"
            let str1 = getCurrentLanguage() == .chinese ? "解散后，该公司下的全部设备自动解除绑定" : "After dismission, all devices under the company are automatically unbound"
    ...
    private lazy var quitButton = ImageTitleButton(frame: .zero, icon: nil, title: "退出公司".localizedString, titleColor: UIColor.custom(.black_333333), backgroundColor: UIColor.custom(.white_ffffff)).then {
        $0.isHidden = true
        $0.clickCallBack = { [weak self] in
            guard let self = self else { return }
            
            let str0 = getCurrentLanguage() == .chinese ? "确定退出吗?\n\n" : "Are you sure to delete it?\n\n"
            let str1 = getCurrentLanguage() == .chinese ? "退出后，不能查看并控制该公司的房间设备" : "After quit, all areas and devices under the family/company will be invisable"
            var attributedString = NSMutableAttributedString(
                string: str0,
                attributes: [
                    NSAttributedString.Key.font : UIFont.font(size: ZTScaleValue(14), type: .bold),
    ...
    private lazy var deleteOrQuitButton = ImageTitleButton(frame: .zero, icon: nil, title: "删除/退出公司".localizedString, titleColor: UIColor.custom(.black_333333), backgroundColor: UIColor.custom(.white_ffffff)).then {
        $0.isHidden = true
        $0.clickCallBack = { [weak self] in
            guard let self = self else { return }
    ...
    private lazy var generateQRCodeAlert = CompanyGenerateQRCodeAlert(frame: CGRect(x: 0, y: 0, width: Screen.screenWidth, height: Screen.screenHeight))
    ...
    override func viewWillAppear(_ animated: Bool) {
        ...
        navigationItem.title = "公司".localizedString
    ...
    override func setupViews() {
        ...
        generateQRCodeAlert.callback = { [weak self] roles, departments in
            guard let self = self else { return }
            let role_ids = roles.map(\.id)
            let department_ids = departments.map(\.id)

            self.getInviteQRCode(role_ids: role_ids, department_ids: department_ids)
        }
...
extension CompanyDetailViewController: UITableViewDelegate, UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        if indexPath.section == 0 {
            ...
            switch infoCellTypes[indexPath.row] {
            case .name:
                let setNameAlertView = InputAlertView(labelText: "公司名称".localizedString, placeHolder: "请输入公司名称".localizedString, limitText: 50) { [weak self] text in
                    guard let self = self else { return }
...
extension CompanyDetailViewController {
    @objc private func requestNetwork() {
        ...
        if area.id == nil && !UserManager.shared.isLogin {
            ...
            departmentsNumCell.valueLabel.text = "\(result.locations_count)"
        ...
        requestQueue.async { [weak self] in
            guard let self = self else { return }
            lazy var sema = DispatchSemaphore(value: 0)
            ...
            ApiServiceManager.shared.areaDetail(area: self.area) { [weak self] (response) in
                ...
                DispatchQueue.main.async { [weak self] in
                    guard let self = self else { return }
                    self.departmentsNumCell.valueLabel.text = "\(response.department_count)"
                ...
                if code == 5012 || code == 5027 { //token失效
                    ...
                    DispatchQueue.main.async { [weak self] in
                        guard let self = self else { return }
                        /// 提示被管理员移除家庭
                        WarningAlert.show(message: "你已被管理员移出公司".localizedString + "\"\(self.area.name)\"")
                        ...
                        if self.authManager.currentArea.id == self.area.id {
                            if let currentArea = AreaCache.areaList().first {
                                ...
                                if UserManager.shared.isLogin { /// 若已登录同步到云端
                                    ApiServiceManager.shared.createArea(name: area.name, location_names: [], department_names: [], area_type: .company) { [weak self] response in
                                        guard let self = self else { return }
                                        DispatchQueue.main.async { [weak self] in
                                            guard let self = self else { return }
                                            area.id = response.id
            ...
            sema.wait()
            /// 获取部门列表
            ApiServiceManager.shared.departmentList(area: self.area) { [weak self] response in
                guard let self = self else { return }
                DispatchQueue.main.async { [weak self] in
                    guard let self = self else { return }
                    self.generateQRCodeAlert.setupDepartments(departments: response.departments)
                    sema.signal()
                }
...
extension CompanyDetailViewController {
    ...
    private func deleteArea(isDeleteDisk: Bool) {
        ...
        if !area.is_bind_sa && area.id == nil {
            ...
            showToast(string: "解散成功".localizedString)
                ...
                if AreaCache.areaList().count == 0 {
                    ...
                    if UserManager.shared.isLogin {
                        self.authManager.syncLocalAreasToCloud(needUpdateCurrentArea: true) { [weak self] in
                            guard let self = self else { return }
                            self.navigationController?.popToRootViewController(animated: true)
                            self.showToast(string: "解散成功".localizedString)
                        }
                    }
...
//                        self.navigationController?.tabBarController?.selectedIndex = 0
    ...
    private func getInviteQRCode(role_ids: [Int], department_ids: [Int]) {
        let areaName = area.name
        
        generateQRCodeAlert.generateButton.selectedChangeView(isLoading: true)
        
        ApiServiceManager.shared.getInviteQRCode(area: area, role_ids: role_ids, department_ids: department_ids) { [weak self] (response) in
            guard let self = self else { return }
...
extension CompanyDetailViewController {
    private func checkAuthState() {
        ...
        if rolePermission.update_area_company_name {
