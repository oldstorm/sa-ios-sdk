...
class MemberInfoCell: UITableViewCell, ReusableView {
    enum MemberInfoCellType {
        case role
        case department
        ...
        var icon: UIImage? {
            switch self {
            case .role:
                return .assets(.icon_role)
            case .department:
                return .assets(.icon_department)
            }
        }
        ...
        var title: String {
            switch self {
            case .role:
                return "角色".localizedString
            case .department:
                return "部门".localizedString
            }
        }
    }
    ...
    convenience init(type: MemberInfoCellType) {
        self.init(style: .default, reuseIdentifier: MemberInfoCell.reusableIdentifier)
    ...
    private func setupViews() {
        ...
        line.snp.makeConstraints {
            $0.top.equalTo(valueLabel.snp.bottom).offset(17.5)
