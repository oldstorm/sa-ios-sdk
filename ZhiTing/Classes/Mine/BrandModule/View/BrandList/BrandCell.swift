...
class BrandCell: UITableViewCell, ReusableView {
    ...
    var updateButtonCallback: (() -> ())?
    var installButtonCallback: (() -> ())?
    ...
    var brand: Brand? {
        didSet {
            ...
            case .chinese:
                pluginNumLabel.text = "共\(brand.plugin_amount)个插件."
            case .english:
                pluginNumLabel.text = "totally \(brand.plugin_amount) plugins."
            }
    ...
    private lazy var pluginNumLabel = Label().then {
        $0.font = .font(size: 12)
    ...
    private func setupViews() {
        ...
        contentView.addSubview(pluginNumLabel)
    ...
    private func setConstrains() {
        ...
        brandNameLabel.snp.makeConstraints {
            $0.top.equalTo(icon.snp.top).offset(1.5)
            $0.left.equalTo(icon.snp.right).offset(15.5)
            $0.right.equalToSuperview().offset(-70)
        }

        pluginNumLabel.snp.makeConstraints {
            $0.top.equalTo(brandNameLabel.snp.bottom).offset(2)
            $0.left.equalTo(brandNameLabel.snp.left)
            $0.right.equalTo(brandNameLabel.snp.right)
